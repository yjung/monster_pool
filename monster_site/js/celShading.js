...
function initialisiereCelShading(){
	...
	window.game.celShading.kontur = true;	// CelShading-Hatching-NameSpace
...
function erstelleCelShadingComposer(kontur){
	...
	// Groesse setzen
	game.composerCelShading.renderPass = new THREE.RenderPass(game.szene, game.kamera);		// Renderpass fuer Szene aus Kamera

	game.composerCelShading.edgePass = new THREE.ShaderPass(CannyEdgePass);			// Custom-Shader
	game.composerCelShading.edgePass.renderToScreen = false;

	game.composerCelShading.effectcopy = new THREE.ShaderPass(THREE.CopyShader);		// Kopier-Shader fuer Effekte, die nicht selbst/direkt gerendert werden koennen
	game.composerCelShading.effectcopy.renderToScreen = true;								// Als Letzten markieren bzw. final auf das Canvas rendern	game.composerCelShading.addPass(effectcopy);

	game.composerCelShading.addPass(game.composerCelShading.renderPass);			// Normales Bild rendern
	game.composerCelShading.addPass(game.composerCelShading.effectcopy);			// Custom-Effekt
	if(kontur){
	game.composerCelShading.addPass(game.composerCelShading.renderPass);			// Normales Bild rendern
	game.composerCelShading.addPass(game.composerCelShading.edgePass);				// Custom-Effekt
	game.composerCelShading.addPass(game.composerCelShading.effectcopy);			// Custom-Effekt				
	}		
...
function celShadingGUIKontur() {
	...
	game.debugGUI.konturfarbe.onChange(function(value) {
		if(game.debugGUI.kontur.object.kontur){
			console.log("1");														// Pruefen ob Kontur aktiv			
			...
			console.log(game.debugGUI.kontur);														// Pruefen ob Kontur aktiv			
			erstelleCelShadingComposer(true);
			var neueFarbe = new THREE.Vector4(value[0]/255,value[1]/255,value[2]/255,value[3]/255);
			game.composerCelShading.edgePass.uniforms.uKonturFarbe.value = neueFarbe;
		}
	...
	celShadingKontur.close(); 	// Ordner standardmaessig oeffnen oder geschlossen
};
